...
class CVDModelPipeline:
    ...
    def load_and_clean(self) -> pd.DataFrame:
        ...
        # CHECK: Do you have a real outcome variable?
        potential_targets = ['CVD_Outcome', 'CVD_Diagnosis', 'Cardiovascular_Disease', 
                            'Heart_Disease', 'MI', 'Stroke', 'CVD_Event']
        
        target_col = None
        for col in potential_targets:
            if col in df.columns:
                target_col = col
                print(f"✅ Found target variable: {col}")
                break
        
        if target_col is None:
            print("⚠️ WARNING: No actual CVD outcome variable found!")
            print("   Available columns:", df.columns.tolist()[:20])
            print("   Will create synthetic risk labels (NOT suitable for clinical use)")
